...
export async function POST(request: Request) {
  try {
    const { name, email, phone, subject, message, storeName, storeEmail } = await request.json()

    if (!name || !email || !subject || !message) {
      return NextResponse.json({ error: "Faltan campos requeridos" }, { status: 400 })
    }

    // Enviar email al dueño de la tienda
    await resend.emails.send({
      from: `${storeName} - Contacto <ventas@tiendaonline.com.ar>`,
      to: storeEmail,
